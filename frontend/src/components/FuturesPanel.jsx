  ...
  return (
    <Panel
      ...
      actions={
        <span className="text-[10px] uppercase tracking-widest text-terminal-muted">
          {curve?.label ?? "—"} · {points.length} contracts
        </span>
      }
    >
    <div className="flex h-full flex-col">
      ...
      {error ? (
        ...
        <div className="mt-2 text-xs text-terminal-muted">Loading curve…</div>
      ) : points.length === 0 ? (
        <div className="mt-2 text-xs text-terminal-muted">
          No back-month contracts returned for {root} — yfinance often skips
        ...
        <div className="mt-2 flex flex-1 min-h-0 flex-col">
          ...
          <table className="mt-2 w-full text-[11px] tabular">
            <thead>
              <tr className="text-left text-[10px] uppercase tracking-widest text-terminal-muted">
                ...
                <th className="py-1 pr-2 text-right">Expiration</th>
          ...
          <p className="mt-2 text-[10px] leading-relaxed text-terminal-muted">
            Curve sloping <span className="text-terminal-amber">up</span> = contango
            (back-months pricier than front, supply abundance). Sloping{" "}
            <span className="text-terminal-amber">down</span> = backwardation
            (front pricier, supply tight). Dashed amber line is the front-month
            continuous price.
          </p>
        </div>
      )}
