...
## 1. Next session — Phase 9 candidate punch list

Phase 8 shipped all four candidates (factor analysis, fixed income,
futures, ESG filings filter). The §5 Phase 9 vision is "mobile + i18n";
the highest-leverage items inside that:

1. **PWA manifest + service worker.** Make the terminal installable on
   iOS / Android / desktop with offline cache for the static shell. Tiny
   code change, big retail-friendly distribution unlock.
2. **Mobile-tuned Launchpad.** The `sm` breakpoint already exists but
   the panel layouts are basically a vertical stack — a real mobile pass
   would prioritize Watchlist / Chart / News on the home screen and
   bury the rest behind a nav drawer.
3. **i18n: EN, ES, PT, ZH.** The roadmap calls out emerging-market UX
   as a deliberate strength. A minimal i18n layer (react-i18next or our
   own lookup table) covers ~80% of strings.
4. **Theme registry.** Dark / light / high-contrast as first-class JSON
   themes, shareable the same way layouts are now. Pairs naturally with
   Phase 7's share infrastructure.
